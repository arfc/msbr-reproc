...
total computational time for achieving equilibrium composition was 
approximately 9,900 node-hours (18 core-years.)

\subsection{Effective multiplication factor}
Figure~\ref{fig:keff} demonstrates the effective multiplication factors 
...
from the protactinium decay tank is added.  \begin{figure}[ht!] % replace 't' 
        with 'b' to \centering
...
the core during reactor operation.  \begin{figure}[ht!] % replace 't' with 'b' 
...
critical.  \begin{figure}[ht!] % replace 't' with 'b' to force it to \centering
  \includegraphics[width=\textwidth]{spectrum_zones.png} \caption{Neutron flux 
...
initial and equilibrium fuel composition. Figure~\ref{fig:pow_den} reflects the 
normalized power distribution of the \gls{MSBR} quarter core which is the same 
at both states. For both the initial and equilibrium compositions, fission 
...
equilibrium, but most of the power is still generated in zone I. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[ht!]
  \centering
...
temperature feedback is an expansion of matterial that is heated. When the fuel 
...
(graphite) rod, safety (B$_4$C) rod, and the whole reactivity control system 
worth. Control rod integral worth is approximately 28 cents and stays almost 
...
absorber accumulation in proximity to reactivity control system rods. This 16\% 
decline in control system worth should be taken into account in \gls{MSBR} 
...
softer for the initial state. Neutron spectrum hardening causes the fast 
...
calculated for 60 years of reactor operation. The the $^{232}$Th feed rate 
...
\begin{figure}[ht!] % replace 't' with 'b' to force it to \centering
  \includegraphics[width=\textwidth]{keff_rem_cases.png} \caption{Calculated 
