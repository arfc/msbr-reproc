...
of cycle time (there is fresh fuel composition at the first step). Next, it 
...
applying feeds and removals. $K_{eff}$ increases accordingly since major reactor 
...
In particular, the $^{233}$U number density fluctuates less than 0.8\% between
...
contains significant amount of $^{235}$U ($\approx10^{-5}$ atom/b-cm), $^{239}$Pu 
...
but notable for thermal neutrons. This neutron flux radial distribution is in 
...
generated in zone I with exception to the majority of $^{233}$Th that is 
produced in zone II.
\begin{figure}[ht!] % replace 't' with 'b' to force it to \centering
...
graphite rods are referred to as ``control'' rods.}, safety (B$_4$C) rods, and 
...
element removal is considerable a few months after startup and achieves 
...
from the fuel salt due to fission and, while, fission products 
