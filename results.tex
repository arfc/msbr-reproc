...
\begin{figure}[ht!] % replace 't' with 'b' to 
  \centering
  \includegraphics[scale=•]{•}=\textwidth]{major_isotopes_adens.png}
...
contained significant amount of $^{235}$U ($\approx10^{-5}$ atom/b-cm), $^{239}$Pu 
...
but notable for thermal neutrons. These neutron flux radial distributions is in 
...
The effective multiplication factor can be expressed using next formula:
\begin{align*}
