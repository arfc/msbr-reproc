...
$^{135}$Xe is a strong poison to the reactor, and some fraction of this gas 
is absorbed by graphite during \gls{MSBR} operation. ORNL calculations shown 
that for unsealed commercial graphite with helium permeability 10$^{-5}$ cm$^2$/s 
the calculated poison fraction is less than 2\% \cite{robertson_conceptual_1971}. 
This parameter can be improved by using experimental graphites or by applying
sealing technology. The effect of the gradual poisoning of the core graphite 
with xenon is not treated here.
% Consequently, minimizing
% xenon absorption in a graphite is desirable. 
%ORNL calculations shown 
%that the graphite must have a gas permeability in the order of 10$^{-8}$ cm$^2$/s 
%to keep the xenon poisoning about 0.5\%. The best commercially available graphites 
%have this parameter in the range from 10$^{-3}$ to 10$^{-4}$ cm$^2$/s, hence, 
%desirable parameters ca be achieved only by sealing the graphite
...
Current version of SaltProc only allows separate out 100\% of specific elements 
or group of elements (e.g. Processing Groups as described in 
Table~\ref{tab:reprocessing_list}) at the end of the specific cycle time. That is 
simplification because in reality fuel salt resides approximately 1/2 of total time 
in the core and rest of the time in the primary loop pipes and the heat exchanger.  
...
for the elements with longer cycle time (i.e. rare earths should be removed 
...
fraction (calculated separately for each reprocessing group) will be removed each
 depletion step or batch (e.g. in the current work it was chosen 3 days).
